...
class app:
    ...
    def __init__(self):
        ...
        parser = argparse.ArgumentParser(
            add_help=False, description="Takes a disk and prints its contents.")

        # Add args
        parser.add_argument("-v", "--version", action="version",
                            version="File Size Distribution Version --> " + version, help="Display current program version")
        parser.add_argument("-d", "-dir", "--directory", action="store_true",
                            help="List byte distribution across folders of the input drive")
        parser.add_argument("-h", "-H", "--help", "-?", action="help",
                            default=argparse.SUPPRESS, help="Show this help message and exit.")
        parser.add_argument("-f", "--file", type=str,
                            help="Specify a drive file")
...
def parseInput():
    ...
    if len(sys.argv) == 1 and sys.stdin.isatty():
        app.parser.print_help(sys.stderr)
        sys.exit(1)
